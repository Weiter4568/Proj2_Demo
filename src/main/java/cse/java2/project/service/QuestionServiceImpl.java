...
public class QuestionServiceImpl implements QuestionService {
  ...
  public String getPercentageOfUnansweredQuestions() {
    Query query =
        em.createNativeQuery(
            "SELECT(CAST(SUM(CASE WHEN answer_count = 0 THEN 1 ELSE 0 END) AS FLOAT) / count(*)) as percentage FROM questions;");
  ...
  public String getAverageAndMaxAnswerCount() {
    Query query =
        em.createNativeQuery(
            "SELECT avg(answer_count) as average, max(answer_count) as maximum FROM questions",
            Tuple.class);
  ...
  public String getAnswerCountDistribution() {
    Query query =
        em.createNativeQuery(
            "SELECT CASE WHEN answer_count = 0 THEN '0' WHEN answer_count = 1 THEN '1' WHEN answer_count BETWEEN 2 AND 5 THEN '2-5' WHEN answer_count BETWEEN 6 AND 10 THEN '6-10' WHEN answer_count BETWEEN 11 AND 20 THEN '11-20' WHEN answer_count BETWEEN 21 AND 30 THEN '21-30' WHEN answer_count BETWEEN 31 AND 40 THEN '31-40' ELSE '40+' END AS answer_range,COUNT(*) AS count FROM questions GROUP BY answer_range;");
  ...
  public String getPercentageOfAcceptedAnswers() {
    Query query =
        em.createNativeQuery(
            "SELECT(CAST(SUM(CASE WHEN answered = true THEN 1 ELSE 0 END) AS FLOAT) / count(*)) as percentage FROM questions;\n");
    List<String> names = new ArrayList<>();
    List<Double> values = new ArrayList<>();
  ...
  public String getPercentageOfQuestionsWhereNonAcceptedAnswerHasHigherScore() {
    Query query =
        em.createNativeQuery(
            "SELECT CAST((SELECT COUNT(*) FROM (SELECT questions.id FROM questions INNER JOIN answers AS accepted_answers ON questions.id = accepted_answers.question_id AND accepted_answers.is_accepted = true LEFT JOIN answers AS other_answers ON questions.id = other_answers.question_id AND other_answers.is_accepted = false WHERE other_answers.score > accepted_answers.score GROUP BY questions.id) AS subquery) AS FLOAT) / (SELECT COUNT(*) FROM questions WHERE answer_count > 0) AS percentage");
    List<String> names = new ArrayList<>();
    List<Double> values = new ArrayList<>();
  ...
  public String getMostUpvotedTags() {
    Query query =
        em.createNativeQuery(
            "SELECT tags, COUNT(*) as tag_frequency FROM (SELECT UNNEST(regexp_split_to_array(tags, E'[,\\\\[\\\\]\\\"]')) as tags FROM questions WHERE tags LIKE '%java%') subquery WHERE tags != 'java' and tags != '' GROUP BY tags ORDER BY tag_frequency DESC;");
    List<Object[]> objects = query.getResultList();
    List<String> names = new ArrayList<>();
    List<Double> values = new ArrayList<>();
  ...
  public String getMostUpvotedTagCombos() {
    ...
    for (Object[] result : results) {
      ...
      List<String> tagList =
          Arrays.asList(tags.split("[,\\[\\]\"]")).stream()
    ...
    List<Map.Entry<String, Integer>> sortedTagCombos =
        tagComboScores.entrySet().stream()
  ...
  public String getUserThreadDistribution() {
    Query query =
        em.createNativeQuery(
            "SELECT post_id, COUNT(DISTINCT user_id)FROM (SELECT id as post_id, user_id FROM questions UNION ALL SELECT question_id as post_id, user_id FROM answers UNION ALL SELECT post_id, user_id FROM comments) AS all_contributions GROUP BY post_id;");
    List<Object[]> results = query.getResultList();
  ...
  public String getUserAnswerCountDistribution() {
    Query query =
        em.createNativeQuery(
            "SELECT post_id, COUNT(Distinct user_id)From (SELECT id as post_id, user_id From questions UNION ALL SELECT question_id as post_id, user_id From answers) as ans_contribution GROUP BY post_id;");
    List<Object[]> results = query.getResultList();
    Map<String, Integer> distribution = new HashMap<>();
    List<String> names = new ArrayList<>();
    List<Double> values = new ArrayList<>();
  ...
  public String getUserCommentCountDistribution() {
    Query query =
        em.createNativeQuery(
            "SELECT post_id, COUNT(Distinct user_id)From (SELECT id as post_id, user_id From questions UNION ALL SELECT post_id as post_id, user_id From comments) as ans_contribution GROUP BY post_id;");
    List<Object[]> results = query.getResultList();
    Map<String, Integer> distribution = new HashMap<>();
    List<String> names = new ArrayList<>();
    List<Double> values = new ArrayList<>();
  ...
  public Map<String, Long> getQuestionCountsPerUser() {
    return questionRepository.countQuestionsPerUser().stream()
        .collect(Collectors.toMap(objs -> objs[1].toString(), objs -> (Long) objs[0]));
  }
  // 获取每个用户的回答数
  ...
  private String getString(
      Map<Integer, Long> distribution, Map<String, Long> commentCountsPerUser) {
  ...
  // A simple method to determine the bucket for a count. Adjust this as needed for your
  // application.
  private int calculateBucket(Long count) {
    return count.intValue(); // This will put counts into buckets of size 10
  }
